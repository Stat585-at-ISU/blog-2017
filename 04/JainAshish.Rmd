...
In order to carry out a number of arithmetic operations with time data, a number of time-related data objects has been supported by *lubridate*.

####Instants
When we capture a specific moment of time, it is called as instant. For example, 14th February 2017, when we parse this into a date in R, it will create an instant having value `2012-02-14 12:00:00`. It is basically a data type to store a particular moment in time. In *lubridate* package,  does not create a new class for instant objects. Instead, it recognizes any date-time object that refers to a moment of time as an instant.
