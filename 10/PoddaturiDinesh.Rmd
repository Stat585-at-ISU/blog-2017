...
We encountered some issues with the data we used in the package. We saved our data as .Rda instead of .rda extension. After changing the extension, our function reads the data successfully. We saved our chicks data in the R workspace using ‘save()’. Later we realized this command would save the data into the object ‘data’ instead of ‘Chicks.rda’. After changing the data object we were able to fix all the data issues. 

In the future, I would save data carefully, write documentation for every function clearly and follow all the necessary steps to build a package.
