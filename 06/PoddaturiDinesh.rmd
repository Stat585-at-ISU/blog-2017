...
```
The above code gives proper results without any errors. If we want to use these functions in multiple places, we need to type/copy the same lines wherever we need them, which results in duplicate code which will eventually lead to bugs in the code. We could make these functions general and call them inside a function like below
```{r}
x <- 1:10
...
```
The above code still has some duplicates (na.rm=TRUE), we can take them out and pass as arguments inside lapply like below
...
```
Finally, we can modify the above function by removing duplicates and make it more general as following
