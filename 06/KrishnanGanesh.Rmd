...
programming as in the first code chunk allows us to generalise the code and reuse it. 

As we saw in the first code chunk using an anonymous function is possible when it’s not worth the effort to give it 
a name. Functional programming makes use of Lexical scoping which determines where to look for values. This is 
helpdul especially because R looks for values when the function is run, not when it’s created, allowing the output 
of a function to be different and depending on objects outside its own environment.
